...
def generate_matrix():###Генерує матрицю
    def f(X1, X2, X3):
        from random import randrange
        y = 1.9 + 7.1 * X1 + 0.4 * X2 + 3.6 * X3 + 3.8 * X1 * X1 + 0.9 * X2 * X2 + 2.8 * X3 * X3 + 7.4 * X1 * X2 + \
            0.2 * X1 * X3 + 2.7 * X2 * X3 + 3.0 * X1 * X2 * X3 + randrange(0, 10) - 5
        return y
...
def x(l1, l2, l3):###Пошук зоряних точок
    x_1 = l1 * delta_x1 + x01
    x_2 = l2 * delta_x2 + x02
    x_3 = l3 * delta_x3 + x03
    return [x_1, x_2, x_3]
...
def a(first, second):  ###Пошук коефіцієнтів а
    need_a = 0
    for j in range(N):
...
def find_known(number):###Пошук коефіціентів а1, а2, ...
    need_a = 0
    for j in range(N):
...
def solve(lst_1, lst_2):###Функція розв'язку СЛАР
    from numpy.linalg import solve
    solver = solve(lst_1, lst_2)
    return solver


def check_result(b_lst, k):###Перевірка знайдених коефіціентів
...
while not adekvat:
    ...
    print("______________________Рівняння регресії з урахуванням квадратичних членів___________________")
    print("y=b0+b1*x1+b2*x2+b3*x3+b12*x1*x2+b13*x1*x3+b23*x2*x3+b123*x1*x2*x3+b11*x1^2+b22*x2^2+b33*x3^2")
    print("_______________________________________________________________Умова за варіантом________________________________________________________________________________________")
    print( "f(х1, х2, х3) = 1.9 + 7.1 * X1 + 0.4 * X2 + 3.6 * X3 + 3.8 * X1 * X1 + 0.9 * X2 * X2 + 2.8 * X3 * X3 + 7.4 * X1 * X2 + 0.2 * X1 * X3 + 2.7 * X2 * X3 + 3.0 * X1 * X2 * X3")
    print("_______________________________________Отримане рівняння регресії______________________________________________________________________________________________")
    ...
    while not odnorid:
        ...
        print("____________________________Критерій Кохрена____________________________")
    ...
    print("___________________________________________Отримане рівняння регресії з урахуванням критерія Стьюдента_________________________________________________________")
    print("{:.3f} + {:.3f} * X1 + {:.3f} * X2 + {:.3f} * X3 + {:.3f} * Х1X2 + {:.3f} * Х1X3 + {:.3f} * Х2X3"
          "+ {:.3f} * Х1Х2X3 + {:.3f} * X11^2 + {:.3f} * X22^2 + {:.3f} * X33^2 = ŷ\n\tПеревірка"
    ...
    print("______________Критерій Фішера_________________")
